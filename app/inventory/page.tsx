...
export default function InventoryPage() {
  ...
  return <main className="container stack">
    <section className="brand-hero">
      <div><div className="eyebrow">INVENTARIO · CABUDARE</div><h1>Existencias</h1><p>Inventario operativo conectado a las órdenes. Los cambios de aceite descuentan aceite y filtro cuando la orden se cierra.</p></div>
    ...
    <section className="stack">
      {filtered.map(r => <article className="card stack" key={r.id}>
        ...
        <div className="row-between">
          <div>{r.current_ref_bcv != null ? <><strong>{fmtRef(r.current_ref_bcv)}</strong><div className="muted small">{fmtVes(r.current_price_ves ?? 0)} · precio catálogo</div></> : <div className="muted small">Sin precio vinculado · se solicitará REF manual al vender</div>}</div>
