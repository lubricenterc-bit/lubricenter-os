...
type OrderRecord = { id: string; order_number: string; status: "OPEN" | "CLOSED" | "CANCELLED"; customer_id: string | null; vehicle_id: string | null; opened_at: string; closed_at: string | null; total_ves: number; total_ref: number };
...
export function OrderWorkspace({ initialOrderId }: { initialOrderId?: string }) {
  ...
  async function ensureOrder() {
    ...
    const newOrder: OrderRecord = { id: row.id, order_number: row.order_number, status: "OPEN", customer_id: null, vehicle_id: null, opened_at: new Date().toISOString(), closed_at: null, total_ves: 0, total_ref: 0 };
    setOrderId(row.id);
    setOrder(newOrder);
  ...
  async function refresh(id = orderId) { if (id) await loadOrder(id, { customers, vehicles }); }
  ...
  async function closePaid() {
    ...
    if (remainingVes > 1) return setError(`Todavía faltan ${fmtVes(remainingVes)}. Agrega el pago o usa Crédito LC.`);
    setBusy(true); setError("");
  ...
  return <div className="stack">
    ...
    {receivable && <section className="card stack" style={{ borderColor: "rgba(255,93,21,.45)" }}><div className="row-between"><strong>Crédito LC</strong><span className={`pill ${receivable.status === "PAID" ? "ok" : "warn"}`}>{receivable.status === "PAID" ? "PAGADO" : "PENDIENTE"}</span></div><div className="money-lg">{fmtVes(receivable.outstanding_ves)}</div><div className="muted small">Saldo original aprox. {fmtRef(receivable.principal_ref)}{receivable.due_date ? ` · vence ${receivable.due_date}` : ""}</div></section>}
    <section className="card stack"><div className="row-between"><div><div className="label">Cliente y vehículo</div>{customer || vehicle ? <div><strong>{customer?.name || customer?.phone || "Cliente sin nombre"}</strong><div className="muted small">{[customer?.phone, vehicle?.plate, vehicle?.make, vehicle?.model, vehicle?.year].filter(Boolean).join(" · ")}</div>{vehicle?.current_odometer != null && <div className="muted small">Kilometraje: {vehicle.current_odometer.toLocaleString("es-VE")} km</div>}</div> : <div className="muted">Asócialo para conservar historial y poder usar Crédito LC.</div>}</div>{!locked && <button className="btn btn-ghost" onClick={() => setModal("PARTY")}>{customer || vehicle ? "Cambiar" : "Asignar"}</button>}</div></section>
    {!locked && <section className="grid grid-2"><button className="btn btn-primary" onClick={() => setModal("PRODUCT")}>+ Producto</button><button className="btn" onClick={() => setModal("SERVICE")}>+ Trabajo / servicio</button></section>}
    <section className="card"><div className="row-between"><h2 className="section-title">Items</h2><span className="muted small">{items.length} líneas</span></div>{items.map(item => <div className="order-item" key={item.id}><div className="row-between"><div><strong>{item.description}</strong><div className="muted small">{item.business_area} · {item.quantity}x</div></div>{!locked && <button className="btn btn-ghost" onClick={() => removeItem(item.id)}>Quitar</button>}</div><div className="row-between"><div><strong>{fmtRef(item.charged_ref_amount)}</strong><div className="muted small">{fmtVes(item.charged_ves_amount)}</div></div>{item.cash_price_revealed && item.cash_usd_special_total != null && <span className="pill">USD físico {fmtRef(item.cash_usd_special_total)}</span>}</div>{(Number(item.worker_share_ref_snapshot || 0) > 0 || Number(item.assistant_bonus_ref_snapshot || 0) > 0) && <div className="muted small">{item.business_area === "WORKSHOP" && <>Cheo: {fmtRef(item.worker_share_ref_snapshot)} · Bono Alexis: {fmtRef(item.assistant_bonus_ref_snapshot)}</>}{item.business_area === "ELECTROAUTO" && <>Alexis: {fmtRef(item.worker_share_ref_snapshot)}</>}</div>}</div>)}{!items.length && <div className="muted">Todavía no hay productos ni trabajos en esta orden.</div>}</section>
    <section className="card stack"><div className="row-between"><strong>Total orden</strong><div style={{ textAlign: "right" }}><div className="money-lg">{fmtRef(totalRef)}</div><div className="muted">{fmtVes(totalVes)}</div></div></div><div className="divider" /><div className="row-between"><strong>Pagado</strong><span>{fmtVes(paidVes)}</span></div><div className="row-between"><strong>Pendiente</strong><div style={{ textAlign: "right" }}><div>{fmtVes(remainingVes)}</div><div className="muted small">{fmtRef(remainingRef)} REF · ${remainingCashUsd.toFixed(2)} USD físico</div></div></div>{!locked && <button className="btn" disabled={!items.length} onClick={() => setModal("PAYMENT")}>+ Agregar pago</button>}{payments.map(p => <div className="row-between" key={p.id}><div><span className="muted small">{paymentLabel(p.method)}</span>{p.reference && <div className="muted small">Ref. {p.reference}</div>}</div><div className="row"><strong>{p.currency === "USD" ? `$${Number(p.amount_original).toFixed(2)}` : fmtVes(p.amount_original)}</strong>{!locked && <button className="btn btn-ghost" onClick={() => removePayment(p.id)}>×</button>}</div></div>)}</section>
    {!locked ? <section className="stack"><button className="btn btn-ghost btn-block" onClick={() => router.push("/orders")}>Guardar y salir</button><button className="btn btn-primary btn-block" disabled={busy || !items.length || remainingVes > 1} onClick={closePaid}>{busy ? "Cerrando…" : "Cobrar y cerrar"}</button>{remainingVes > 1 && <button className="btn btn-block" disabled={busy || !items.length || !customer} onClick={() => setModal("CREDIT")}>Cerrar con Crédito LC · {fmtVes(remainingVes)}</button>}{remainingVes > 1 && !customer && <div className="muted small" style={{ textAlign: "center" }}>Para Crédito LC primero asigna un cliente.</div>}{orderId && <button className="btn btn-ghost btn-block" disabled={busy} style={{ borderColor: "rgba(255,80,80,.45)", color: "#ff8b8b" }} onClick={deleteOrder}>Eliminar orden duplicada</button>}</section> : <button className="btn btn-ghost btn-block" onClick={() => router.push("/orders")}>Volver a órdenes</button>}
...
function paymentLabel(method: string) { return ({ MOBILE_PAYMENT: "Pago móvil", TRANSFER_BDV: "Transferencia BDV", TRANSFER_BNC: "Transferencia BNC", CASH_VES: "Efectivo Bs", CASH_USD: "Efectivo USD" } as Record<string,string>)[method] ?? method; }

function PartySheet({ customers, vehicles, currentCustomer, currentVehicle, ensureOrder, reloadDirectory, onDone, onCancel }: { customers: Customer[]; vehicles: Vehicle[]; currentCustomer: Customer | null; currentVehicle: Vehicle | null; ensureOrder: () => Promise<string>; reloadDirectory: () => Promise<Directory>; onDone: (customer: Customer | null, vehicle: Vehicle | null, orderId: string) => void | Promise<void>; onCancel: () => void; }) {
  const [mode, setMode] = useState<"EXISTING" | "NEW">("EXISTING");
  const [search, setSearch] = useState(""); const [customerId, setCustomerId] = useState(currentCustomer?.id ?? ""); const [vehicleId, setVehicleId] = useState(currentVehicle?.id ?? "");
  const [name, setName] = useState(""); const [phone, setPhone] = useState(""); const [documentId, setDocumentId] = useState(""); const [plate, setPlate] = useState(""); const [make, setMake] = useState(""); const [model, setModel] = useState(""); const [year, setYear] = useState(""); const [engine, setEngine] = useState(""); const [odometer, setOdometer] = useState("");
  ...
  const customerVehicles = vehicles.filter(v => customerId && v.customer_id === customerId); const selectedVehicle = vehicles.find(v => v.id === vehicleId) ?? null;
  function chooseCustomer(id: string) { setCustomerId(id); if (!vehicles.some(v => v.id === vehicleId && v.customer_id === id)) setVehicleId(""); }
  function chooseVehicle(id: string) { const v = vehicles.find(x => x.id === id); setVehicleId(id); if (v?.customer_id) setCustomerId(v.customer_id); }
  async function saveExisting() { if (!customerId && !vehicleId) return setError("Selecciona al menos un cliente o vehículo."); setBusy(true); setError(""); try { const id = await ensureOrder(); const c = customers.find(x => x.id === customerId) ?? (selectedVehicle?.customer_id ? customers.find(x => x.id === selectedVehicle.customer_id) ?? null : null); const v = vehicles.find(x => x.id === vehicleId) ?? null; const { error } = await supabase.rpc("set_order_party", { p_order_id: id, p_customer_id: c?.id ?? null, p_vehicle_id: v?.id ?? null }); if (error) throw error; await onDone(c, v, id); } catch (e:any) { setError(e.message ?? String(e)); } finally { setBusy(false); } }
  async function saveNew() { setBusy(true); setError(""); try { let newCustomerId:string|null=null; let newVehicleId:string|null=null; if (name.trim() || phone.trim() || documentId.trim()) { const { data,error }=await supabase.rpc("upsert_customer",{p_name:name||null,p_phone:phone||null,p_document_id:documentId||null}); if(error) throw error; newCustomerId=data as string; } if (plate.trim() || make.trim() || model.trim()) { const {data,error}=await supabase.rpc("upsert_vehicle",{p_customer_id:newCustomerId,p_plate:plate||null,p_make:make||null,p_model:model||null,p_year:year?Number(year):null,p_engine:engine||null,p_current_odometer:odometer?Number(odometer):null}); if(error) throw error; newVehicleId=data as string; } if(!newCustomerId&&!newVehicleId) throw new Error("Ingresa datos del cliente o vehículo."); const id=await ensureOrder(); const {error}=await supabase.rpc("set_order_party",{p_order_id:id,p_customer_id:newCustomerId,p_vehicle_id:newVehicleId}); if(error) throw error; const fresh=await reloadDirectory(); await onDone(fresh.customers.find(x=>x.id===newCustomerId)??null,fresh.vehicles.find(x=>x.id===newVehicleId)??null,id); } catch(e:any){setError(e.message??String(e));} finally{setBusy(false);} }
  return <div className="overlay"><div className="sheet stack"><div className="row-between"><h2 style={{margin:0}}>Cliente y vehículo</h2><button className="btn btn-ghost" onClick={onCancel}>Cerrar</button></div><div className="segmented"><button className={`btn ${mode==="EXISTING"?"btn-primary":"btn-ghost"}`} onClick={()=>setMode("EXISTING")}>Buscar existente</button><button className={`btn ${mode==="NEW"?"btn-primary":"btn-ghost"}`} onClick={()=>setMode("NEW")}>Crear nuevo</button></div>{mode==="EXISTING"?<><label><span className="label">Buscar cliente</span><input className="input" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Nombre, teléfono o cédula" autoFocus/></label><div className="stack directory-list">{filteredCustomers.map(c=><button key={c.id} className={`directory-option ${customerId===c.id?"selected":""}`} onClick={()=>chooseCustomer(c.id)}><strong>{c.name||"Cliente sin nombre"}</strong><span>{[c.phone,c.document_id].filter(Boolean).join(" · ")||"Sin datos adicionales"}</span></button>)}</div>{customerId&&<div className="stack"><div className="label">Vehículos del cliente</div>{customerVehicles.map(v=><button key={v.id} className={`directory-option ${vehicleId===v.id?"selected":""}`} onClick={()=>chooseVehicle(v.id)}><strong>{v.plate||`${v.make??""} ${v.model??""}`.trim()||"Vehículo"}</strong><span>{[v.make,v.model,v.year,v.current_odometer!=null?`${v.current_odometer.toLocaleString("es-VE")} km`:null].filter(Boolean).join(" · ")}</span></button>)}</div>}<button className="btn btn-primary btn-block" disabled={busy||(!customerId&&!vehicleId)} onClick={saveExisting}>{busy?"Guardando…":"Usar en esta orden"}</button></>:<><div className="card stack"><strong>Cliente</strong><div className="grid grid-2"><label><span className="label">Nombre</span><input className="input" value={name} onChange={e=>setName(e.target.value)}/></label><label><span className="label">Teléfono</span><input className="input" value={phone} onChange={e=>setPhone(e.target.value)}/></label></div><label><span className="label">Cédula / documento</span><input className="input" value={documentId} onChange={e=>setDocumentId(e.target.value)}/></label></div><div className="card stack"><strong>Vehículo</strong><div className="grid grid-2"><label><span className="label">Placa</span><input className="input" value={plate} onChange={e=>setPlate(e.target.value.toUpperCase())}/></label><label><span className="label">Kilometraje</span><input className="input" type="number" value={odometer} onChange={e=>setOdometer(e.target.value)}/></label><label><span className="label">Marca</span><input className="input" value={make} onChange={e=>setMake(e.target.value)}/></label><label><span className="label">Modelo</span><input className="input" value={model} onChange={e=>setModel(e.target.value)}/></label><label><span className="label">Año</span><input className="input" type="number" value={year} onChange={e=>setYear(e.target.value)}/></label><label><span className="label">Motor</span><input className="input" value={engine} onChange={e=>setEngine(e.target.value)}/></label></div></div><button className="btn btn-primary btn-block" disabled={busy} onClick={saveNew}>{busy?"Guardando…":"Crear y usar"}</button></>}{error&&<div className="error">{error}</div>}</div></div>;
}
...
function ProductSheet({ inventoryProducts, catalogProducts, rates, ensureOrder, onDone, onCancel }: { inventoryProducts: InventoryProduct[]; catalogProducts: CatalogProduct[]; rates: Rates; ensureOrder: () => Promise<string>; onDone: (id:string)=>void|Promise<void>; onCancel:()=>void; }) {
  ...
  const q=search.trim().toLowerCase(); const filteredInventory=inventoryProducts.filter(p=>`${p.sku} ${p.brand??""} ${p.description} ${p.catalog_product_name??""} ${p.category??""}`.toLowerCase().includes(q)).slice(0,60); const filteredCatalog=catalogProducts.filter(p=>`${p.filter_code??""} ${p.name} ${p.category??""}`.toLowerCase().includes(q)).slice(0,60);
