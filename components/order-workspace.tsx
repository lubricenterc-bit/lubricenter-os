...
export function OrderWorkspace({ initialOrderId }: { initialOrderId?: string }) {
  ...
  return <div className="stack">
    ...
    <section className="card stack"><div className="row-between"><div><div className="label">Cliente y vehículo</div>{customer || vehicle ? <div><strong>{customer?.name || customer?.phone || "Cliente sin nombre"}</strong><div className="muted small">{[customer?.phone, vehicle?.plate, vehicle?.make, vehicle?.model, vehicle?.year].filter(Boolean).join(" · ")}</div>{vehicle?.current_odometer != null && <div className="muted small">Kilometraje: {vehicle.current_odometer.toLocaleString("es-VE")} km</div>}</div> : <div className="muted">Asócialo para conservar historial y poder usar Crédito LC.</div>}</div>{!locked && <button className="btn btn-ghost" onClick={() => setModal("PARTY")}>{customer || vehicle ? "Cambiar / nuevo vehículo" : "Buscar o crear"}</button>}</div></section>
