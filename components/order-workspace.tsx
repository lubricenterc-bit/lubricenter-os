...
type Item = { price_denomination:string;agreed_usd:number|null; id: string; item_type: string; business_area: string; description: string; quantity: number; charged_ves_amount: number; charged_ref_amount: number; cash_usd_special_total: number | null; cash_price_revealed: boolean; worker_share_ref_snapshot: number | null; assistant_bonus_ref_snapshot: number | null };
...
type Modal = "PARTY" | "PRODUCT" | "SERVICE" | "PAYMENT" | "CREDIT" | "PARTIAL_CREDIT" | "CASHEA" | null;
...
export function OrderWorkspace({ initialOrderId }: { initialOrderId?: string }) {
  ...
  const orderRates = items.length && items.every(i=>i.price_denomination==="USD") ? {...rates,operative:items.reduce((n,i)=>n+Number(i.charged_ves_amount),0)/items.reduce((n,i)=>n+Number(i.agreed_usd),0)} : rates;
  ...
  const remainingCashUsd = orderRates.operative > 0 ? remainingVes / orderRates.operative : 0;
  ...
  async function loadOrder(id: string) {
    const [{ data: o, error: oe }, { data: its, error: ie }, { data: pays, error: pe }, { data: rec, error: re }, { data: cs, error: cse }] = await Promise.all([
      ...
      supabase.from("order_items").select("id,price_denomination,agreed_usd,item_type,business_area,description,quantity,charged_ves_amount,charged_ref_amount,cash_usd_special_total,cash_price_revealed,worker_share_ref_snapshot,assistant_bonus_ref_snapshot").eq("order_id", id).order("created_at"),
  ...
  return <div className="stack">
    ...
    <section className="card"><div className="row-between"><h2 className="section-title">Productos y trabajos</h2><span className="muted small">{items.length} líneas</span></div>{items.map(item => <div className="order-item" key={item.id}><div className="row-between"><div><strong>{item.description}</strong><div className="muted small">{item.business_area} · {item.quantity}x</div></div>{!locked && <button className="btn btn-ghost" onClick={() => removeItem(item.id)}>Quitar</button>}</div><div className="row-between"><div><strong>{item.price_denomination==="USD"?`${Number(item.agreed_usd).toFixed(2)} USD pactado`:fmtRef(item.charged_ref_amount)}</strong><div className="muted small">{fmtVes(item.charged_ves_amount)}</div></div>{item.cash_price_revealed && item.cash_usd_special_total != null && <span className="pill">USD físico {fmtRef(item.cash_usd_special_total)}</span>}</div>{(Number(item.worker_share_ref_snapshot || 0) > 0 || Number(item.assistant_bonus_ref_snapshot || 0) > 0) && <div className="muted small">{item.business_area === "WORKSHOP" && <>Cheo: {fmtRef(item.worker_share_ref_snapshot)} · Bono Alexis: {fmtRef(item.assistant_bonus_ref_snapshot)}</>}{item.business_area === "ELECTROAUTO" && <>Alexis: {fmtRef(item.worker_share_ref_snapshot)}</>}</div>}</div>)}{!items.length && <div className="muted">Todavía no hay productos ni trabajos en esta orden.</div>}</section>
    ...
    {!locked ? <section className="card stack">{readinessError && <div className="error" role="alert"><strong>Para cerrar:</strong> {readinessError}{/cliente|vehículo/i.test(readinessError) ? <button className="btn btn-block" onClick={() => setModal("PARTY")}>Completar cliente y vehículo</button> : /aceite/i.test(readinessError) ? <div>Quita la línea de servicio incompleta y usa “Cambio de aceite completo”.</div> : <div>Completa las observaciones en “Salida, seguimiento CRM y cortesías”.</div>}</div>}<button className="btn btn-ghost btn-block" onClick={() => router.push("/orders")}>Volver a órdenes · cambios guardados</button><button className="btn btn-block" disabled={busy || !items.length} onClick={() => setModal("CASHEA")}>Cobrar con Cashea</button><button className="btn btn-primary btn-block" disabled={busy || !items.length} onClick={closePaid}>{busy ? "Cerrando…" : remainingVes > 1 ? `Cobrar total · ${fmtVes(remainingVes)}` : "Cerrar orden pagada"}</button>{remainingVes > 1 && <button className="btn btn-block" disabled={busy || !items.length || !customer} style={{ border: "1px solid rgba(255,106,26,.72)", background: "rgba(255,106,26,.12)" }} onClick={() => setModal("PARTIAL_CREDIT")}>Abono + saldo a Crédito LC</button>}{paidVes - totalVes > 1 && <div className="error" role="alert">Pago excedido por {fmtVes(paidVes - totalVes)}. Quita el pago incorrecto antes de cerrar.</div>}{!items.length && <div className="muted">Agrega productos o trabajos antes de cobrar.</div>}{remainingVes > 1 && <button className="btn btn-block" disabled={busy || !items.length || !customer} onClick={() => setModal("CREDIT")}>Dejar todo en Crédito LC · {fmtVes(remainingVes)}</button>}{remainingVes > 1 && !customer && <div className="muted small" style={{ textAlign: "center" }}>Para Crédito LC primero asigna un cliente.</div>}{orderId && <button className="btn btn-ghost btn-block" disabled={busy} style={{ borderColor: "rgba(255,80,80,.45)", color: "#ff8b8b" }} onClick={deleteOrder}>Eliminar orden duplicada</button>}</section> : <button className="btn btn-ghost btn-block" onClick={() => router.push("/orders")}>Volver a órdenes</button>}
    ...
    {modal === "PRODUCT" && <ProductSheet inventoryProducts={inventoryProducts} catalogProducts={catalogProducts} rates={orderRates} ensureOrder={ensureOrder} onDone={async id => { setModal(null); try { await refresh(id); setNotice("Guardado en la orden."); } catch (e: any) { setError(`La operación se guardó, pero no se pudo actualizar: ${e.message}. Recarga la orden; no la registres de nuevo.`); } }} onCancel={() => setModal(null)} />}
    ...
    {modal === "PAYMENT" && <PaymentSheet onCashea={() => setModal("CASHEA")} ensureOrder={ensureOrder} remainingVes={remainingVes} rates={orderRates} onDone={async id => { setModal(null); try { await refresh(id); setNotice("Guardado en la orden."); } catch (e: any) { setError(`La operación se guardó, pero no se pudo actualizar: ${e.message}. Recarga la orden; no la registres de nuevo.`); } }} onCancel={() => setModal(null)} />}
    ...
    {modal === "PARTIAL_CREDIT" && orderId && <PartialCreditSheet orderId={orderId} customer={customer} remainingVes={remainingVes} remainingRef={remainingRef} rates={orderRates} onDone={() => { setModal(null); router.push("/orders"); router.refresh(); }} onCancel={() => setModal(null)} />}
  </div>;
}

function paymentLabel(method: string) { return ({ MOBILE_PAYMENT: "Pago móvil · Banco de Venezuela", TRANSFER_BDV: "Pago móvil · Banco de Venezuela", TRANSFER_BNC: "Pago móvil · BNC", CASH_VES: "Efectivo Bs", ZELLE:"Zelle USD",BINANCE:"Binance USD",CASH_USD: "Efectivo USD" } as Record<string,string>)[method] ?? method; }
...
function ServiceSheet({ ensureOrder, onDone, onCancel }: { ensureOrder:()=>Promise<string>; onDone:(id:string)=>void|Promise<void>; onCancel:()=>void; }) {
  ...
  function displayAmount(value:number){return denomination==="USD"?`${Number(value||0).toFixed(2)} USD`:fmtRef(value);}
  async function add(){if(busy||!Number.isFinite(baseRef)||!Number.isFinite(customerRef)||!description.trim()||baseRef<=0||customerRef<0||preview.error)return;setBusy(true);setError("");try{const id=await ensureOrder();const{error}=await supabase.rpc("add_service_priced",{p_denomination:denomination,p_order_id:id,p_business_area:area,p_description:description.trim(),p_base_ref:baseRef,p_customer_ref:customerRef,p_worker_share_override_ref:workerOverride===""?null:Number(workerOverride),p_alexis_bonus_enabled:area==="WORKSHOP"?bonus:false});if(error)throw error;await onDone(id);}catch(e:any){setError(e.message??String(e));}finally{setBusy(false);}}
  return <div className="overlay"><div className="sheet stack"><div className="row-between"><h2 style={{margin:0}}>Trabajo / servicio</h2><button className="btn btn-ghost" disabled={busy} onClick={onCancel}>Cerrar</button></div><label><span className="label">Área</span><select className="select" value={area} onChange={e=>setArea(e.target.value as any)}><option value="WORKSHOP">Taller · Cheo</option><option value="ELECTROAUTO">Electroauto · Alexis</option></select></label><label><span className="label">Precio acordado</span><select className="select" value={denomination} onChange={e=>setDenomination(e.target.value)}><option value="REF">Referencia BCV · base en bolívares</option><option value="USD">USD pactado · dólares reales</option></select><span className="muted small">{denomination==="USD"?"Si acordaste $40, registra 40. Base y comisiones se calculan en esta misma denominación.":"El monto REF se convierte a bolívares a tasa BCV."}</span></label><label><span className="label">Descripción</span><input className="input" value={description} onChange={e=>setDescription(e.target.value)} placeholder="Ej. Cambio de aceite y filtro"/></label><div className="grid grid-2"><label><span className="label">Base {denomination}</span><input className="input" type="number" min="0" step="0.01" value={baseRef||""} onChange={e=>setBaseRef(Number(e.target.value))}/></label><label><span className="label">Cobro al cliente {denomination}</span><input className="input" type="number" min="0" step="0.01" value={customerRef||""} onChange={e=>setCustomerRef(Number(e.target.value))}/></label></div>{area==="WORKSHOP"&&<label className="row"><input type="checkbox" checked={bonus} onChange={e=>setBonus(e.target.checked)}/><span>Bono meritorio Alexis 5% (sale de Lubricenter)</span></label>}{area!=="OIL_CHANGE"&&<button className="btn btn-ghost" onClick={()=>setAdvanced(v=>!v)}>{advanced?"Ocultar ajuste":"Ajustar reparto excepcional"}</button>}{advanced&&area!=="OIL_CHANGE"&&<label><span className="label">Parte acordada {area==="WORKSHOP"?"Cheo":"Alexis"} {denomination}</span><input className="input" type="number" min="0" step="0.01" value={workerOverride} onChange={e=>setWorkerOverride(e.target.value)} placeholder="Vacío = 40% automático"/></label>}<div className="card">{preview.error?<div className="error">{preview.error}</div>:area==="WORKSHOP"?<div className="stack"><div className="row-between"><span>Cliente</span><strong>{displayAmount(preview.customerRef)}</strong></div><div className="row-between"><span>Cheo</span><strong>{displayAmount(preview.cheoRef)}</strong></div><div className="row-between"><span>Bono Alexis</span><strong>{displayAmount(preview.alexisBonusRef)}</strong></div><div className="row-between"><span>Lubricenter</span><strong>{displayAmount(preview.lubricenterRef)}</strong></div></div>:area==="ELECTROAUTO"?<div className="stack"><div className="row-between"><span>Cliente</span><strong>{displayAmount(preview.customerRef)}</strong></div><div className="row-between"><span>Alexis</span><strong>{displayAmount(preview.alexisRef)}</strong></div><div className="row-between"><span>Lubricenter</span><strong>{displayAmount(preview.lubricenterRef)}</strong></div></div>:<div className="row-between"><span>Total servicio</span><strong>{displayAmount(customerRef)}</strong></div>}</div>{error&&<div className="error">{error}</div>}<button className="btn btn-primary btn-block" disabled={busy||!!preview.error||!description.trim()||baseRef<=0} onClick={add}>{busy?"Agregando…":"Agregar trabajo"}</button></div></div>;
}
...
function PaymentSheet({ onCashea, ensureOrder, remainingVes, rates, onDone, onCancel }: { onCashea:()=>void; ensureOrder:()=>Promise<string>; remainingVes:number; rates:Rates; onDone:(id:string)=>void|Promise<void>; onCancel:()=>void; }) {
  const [method,setMethod]=useState("TRANSFER_BDV"); const currency=["CASH_USD","ZELLE","BINANCE"].includes(method)?"USD":"VES"; const suggested=currency==="USD"?(rates.operative?remainingVes/rates.operative:0):remainingVes; const [amount,setAmount]=useState(0); const [reference,setReference]=useState(""); const [error,setError]=useState(""); const [busy,setBusy]=useState(false); useEffect(()=>{setAmount(Number(suggested.toFixed(2)));},[method,remainingVes,rates.operative]);
  async function add(){if(busy||!Number.isFinite(amount)||amount<=0)return;const re=referenceError(method,reference);if(re)return setError(re);setBusy(true);setError("");try{const id=await ensureOrder();const{error}=await supabase.rpc("add_payment",{p_order_id:id,p_method:method,p_amount_original:amount,p_reference:reference||null});if(error)throw error;await onDone(id);}catch(e:any){setError(e.message??String(e));}finally{setBusy(false);}}
  return <div className="overlay"><div className="sheet stack"><div className="row-between"><h2 style={{margin:0}}>Agregar pago</h2><button className="btn btn-ghost" disabled={busy} onClick={onCancel}>Cerrar</button></div><button className="btn btn-primary" onClick={onCashea} disabled={busy}>Cashea · inicial y 3 cuotas</button><label><span className="label">Pago de contado / abono</span><select className="select" value={method} onChange={e=>setMethod(e.target.value)}><option value="TRANSFER_BDV">Pago móvil · Banco de Venezuela</option><option value="TRANSFER_BNC">Pago móvil · BNC</option><option value="CASH_VES">Efectivo Bs</option><option value="CASH_USD">Efectivo USD físico</option><option value="ZELLE">Zelle USD</option><option value="BINANCE">Binance USD</option></select></label><label><span className="label">Monto {currency}</span><input className="input" type="number" min="0" step="0.01" value={amount||""} onChange={e=>setAmount(Number(e.target.value))}/></label>{method!=="CASH_USD"&&method!=="CASH_VES"&&<label><span className="label">{method.startsWith("TRANSFER")?"Últimos 4 de referencia · obligatorio":"Referencia · opcional"}</span><input className="input" value={reference} onChange={e=>setReference(e.target.value)}/></label>}<div className="card"><div className="muted small">VALOR DEL PAGO</div><div className="money-lg">{fmtVes(currency==="USD"?amount*rates.operative:amount)}</div><div className="muted small">Pagos en USD usan la tasa operativa del momento.</div></div>{error&&<div className="error">{error}</div>}<button className="btn btn-primary btn-block" disabled={busy||amount<=0} onClick={add}>{busy?"Agregando…":"Agregar pago"}</button></div></div>;
}
...
function PartialCreditSheet({ orderId, customer, remainingVes, remainingRef, rates, onDone, onCancel }: { orderId:string; customer:Customer|null; remainingVes:number; remainingRef:number; rates:Rates; onDone:()=>void; onCancel:()=>void; }) {
  const [method,setMethod]=useState("TRANSFER_BDV"); const currency=["CASH_USD","ZELLE","BINANCE"].includes(method)?"USD":"VES"; const maxAmount=currency==="USD"?(rates.operative?remainingVes/rates.operative:0):remainingVes; const [amount,setAmount]=useState(0); const [reference,setReference]=useState(""); const [dueDate,setDueDate]=useState(""); const [error,setError]=useState(""); const [busy,setBusy]=useState(false);
  useEffect(()=>{setAmount(Number((maxAmount*.5).toFixed(2)));},[method,remainingVes,rates.operative]);
  const paidVes=currency==="USD"?amount*rates.operative:amount; const pendingVes=Math.max(remainingVes-paidVes,0); const pendingRef=rates.bcv?pendingVes/rates.bcv:0;
  async function confirm(){
    if(!customer)return setError("Debes asociar un cliente antes de usar Crédito LC.");
    if(!Number.isFinite(amount)||amount<=0)return setError("Indica el abono que recibiste.");
    const re=referenceError(method,reference);if(re)return setError(re);
    if(paidVes>=remainingVes-1)return setError("Para cerrar todo como pagado usa “Cobrar total”. El abono debe dejar un saldo pendiente.");
    setBusy(true);setError("");
    const{error}=await supabase.rpc("close_order_with_partial_credit",{p_order_id:orderId,p_method:method,p_amount_original:amount,p_reference:reference.trim()||null,p_due_date:dueDate||null});
    setBusy(false);if(error)return setError(error.message);onDone();
  }
  return <div className="overlay"><div className="sheet stack"><div className="row-between"><div><h2 style={{margin:0}}>Abono + Crédito LC</h2><div className="muted small">Registra lo recibido y deja el resto a nombre del cliente.</div></div><button className="btn btn-ghost" disabled={busy} onClick={onCancel}>Cancelar</button></div><div className="card stack" style={{borderColor:"rgba(255,106,26,.65)"}}><div className="muted small">CLIENTE</div><strong>{customer?.name||customer?.phone||"Sin cliente"}</strong><div className="divider"/><div className="row-between"><span>Total pendiente actual</span><div style={{textAlign:"right"}}><strong>{fmtVes(remainingVes)}</strong><div className="muted small">{fmtRef(remainingRef)}</div></div></div></div><div><span className="label">Cómo recibiste el abono</span><div className="grid grid-2">{[["TRANSFER_BDV","Pago móvil · Venezuela"],["TRANSFER_BNC","Pago móvil · BNC"],["CASH_VES","Efectivo Bs"],["CASH_USD","Efectivo USD"],["ZELLE","Zelle USD"],["BINANCE","Binance USD"]].map(([value,label])=><button type="button" key={value} className={method===value?"btn btn-primary":"btn"} onClick={()=>setMethod(value)}>{method===value?`✓ ${label}`:label}</button>)}</div></div><label><span className="label">Abono recibido · {currency}</span><input className="input" type="number" min="0.01" step="0.01" max={maxAmount} value={amount||""} onChange={e=>setAmount(Number(e.target.value))}/></label>{method.startsWith("TRANSFER")&&<label><span className="label">Últimos 4 de referencia · obligatorio</span><input className="input" value={reference} onChange={e=>setReference(e.target.value)}/></label>}<label><span className="label">Fecha prometida de pago · opcional</span><input className="input" type="date" value={dueDate} onChange={e=>setDueDate(e.target.value)}/></label><div className="grid grid-2"><div className="card"><div className="muted small">ABONO QUE ENTRA HOY</div><strong>{fmtVes(paidVes)}</strong></div><div className="card" style={{borderColor:"rgba(255,106,26,.55)"}}><div className="muted small">SALDO CRÉDITO LC</div><strong>{fmtVes(pendingVes)}</strong><div className="muted small">{fmtRef(pendingRef)}</div></div></div>{error&&<div className="error">{error}</div>}<button className="btn btn-primary btn-block" disabled={busy||!customer||amount<=0||pendingVes<=1} onClick={confirm}>{busy?"Registrando…":"Cobrar abono y cerrar con Crédito LC"}</button></div></div>;
}
