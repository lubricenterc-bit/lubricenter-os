...
function QuickPartialCreditSheet({ items, totalVes, totalRef, rates, saleDate, onCancel, onDone }: { items:QuickPayloadLine[]; totalVes:number; totalRef:number; rates:Rates; saleDate:string; onCancel:()=>void; onDone:(row:any)=>void|Promise<void> }) {
  ...
  const currency=method==="CASH_USD"?"USD":"VES"; const maxAmount=currency==="USD"?(rates.operative?totalVes/rates.operative:0):totalVes; const paidVes=currency==="USD"?amount*rates.operative:amount; const pendingVes=Math.max(totalVes-paidVes,0); const pendingRef=rates.bcv?pendingVes/rates.bcv:0;
